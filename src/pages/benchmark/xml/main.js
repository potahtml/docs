(function () {
	...
	const PrototypeArray = Array$1.prototype;
	...
	class ReactiveArray extends Array {
	  ...
	  push(...items) {
	    ...
	    // add keys
	    for (let key = this.length, item = 0; key < this.length + items.length; key++, item++) {
	  ...
	  splice(start, deleteCount, ...items) {
	    ...
	    const r = items.length ? super.splice(start, deleteCount, ...items) : deleteCount !== undefined ? super.splice(start, deleteCount) : super.splice(start);
	  ...
	  lastIndexOf(searchElement, fromIndex) {
	    const key = super.lastIndexOf(mutable(searchElement), fromIndex === undefined ? this.length - 1 : fromIndex);
	    trackKey(this, key);
	    return key;
	  }
	  ...
	  *values() {
	  ...
	  toSpliced(start, deleteCount, ...items) {
	    this[$track].read();
	    items = items.map(mutable);
	    return items.length ? super.toSpliced(start, deleteCount, ...items) : deleteCount !== undefined ? super.toSpliced(start, deleteCount) : super.toSpliced(start);
	  }
	  with(key, value) {
	    key = key < 0 ? key + this.length : key;
	    trackKey(this, key);
	...
	const methodsBlacklist = [...getOwnValues(ReactiveMap.prototype), ...getOwnValues(ReactiveArray.prototype)];
