;(function () {
	...
	const toValues = value =>
		isArray(value)
			...
			: isObject(value) && 'values' in value
				? /** @type {{ values(): IterableIterator<T> }} */ value.values()
	 ...
	 */
	const withState = /* #__NO_SIDE_EFFECTS__ */ (
		fn,
		state = cacheStore,
	) => fn.bind(null, state())
	...
	const classListAdd = (node, className) =>
		node.classList.add(className)
	const classListRemove = (node, className) =>
	 ...
	 */
	const adoptedStyleSheetsGet = document =>
	 ...
	 */
	const adoptedStyleSheetsAdd = (document, styleSheet) =>
	 ...
	 * @param {CSSStyleSheet} styleSheet
	 */
	const adoptedStyleSheetsRemove = (document, styleSheet) =>
	 ...
	 * - Writing to a signal returns `bollean` to tell if the value
	 *   changed
	 ...
	 * - `update` function on Signal that could be used to use the old
	 *   value to set a new value
	 * - The system is wrapped in a `createReactiveSystem` function so
	 *   many systems can be run at the same time, for example for the
	 *   developer tools context, so dev-tools context doesnt mess up
	 *   the real context
	 */
	...
	function createReactiveSystem() {
		...
		class Memo extends Computation {
			...
			read = markReactive(() => {
				...
				if (Listener) {
					const sourceSlot = this.observers
		 ...
		 *
		 * @template T
		 * @param {() => T} fn - Function to re-run when dependencies
		 *   change
		 * @param {SignalOptions} [options]
		 */
		...
		/**
		 * Creates a context and returns a function to get or set the
		 * value
		 *
		 * @template T
		...
		function Context(defaultValue = undefined) {
			...
			useContext.Provider = props =>
				useContext(props.value, () =>
					useContext.toHTML(props.children),
				)

			/**
			 * Maps context following `parent` property (if any). When
			 * `true` is returned from the callback it stops walking.
			 *
	...
	const { cleanup, Context, effect, owned, root, signal, untrack } =
	 ...
	 * callbacks. It queues a microtask, then the callbacks are added to
	 * a position in the array. These are run with a priority.
	 */
	 ...
	 *   needed props not being set, or children elements not created
	 *   yet. Default is `true`
	 ...
	 *   - Function to run when this prop is found on any Element
	 *
	 * @param {boolean} [onMicrotask=true] - Set to run on a microtask
	...
	function createTag(tagName, props) {
		/**
		 * Namespace, use props xmlns or special case svg, math, etc in
		 * case of missing xmlns attribute
		 */
	...
	function createChildren(
		...
		switch (typeof child) {
			...
			case 'function': {
				...
				/**
				 * A placeholder is created and added to the document but
				 * doesnt form part of the children. The placeholder needs to
				 * be returned so it forms part of the group of children. If
			...
			case 'object': {
				...
				if (child instanceof Node) {
					/**
					 * DocumentFragment are special as only the children get
					 * added to the document and the document becomes empty. If
					 * we dont insert them 1 by 1 then we wont have a reference
					 * to them for deletion on cleanup with node.remove()
					 */
				...
				if (child instanceof CSSStyleSheet) {
					 ...
					 * connected. So our stylesheet would end on the main
					 * document instead of the shadowRoot
					 */
					onFixes(() => {
	...
	function toHTML(children) {
		/**
		 * DocumentFragment is transformed to an `Array` of
		 * `Node/Element`, that way we can keep a reference to the nodes.
		 * Because when the DocumentFragment is used, it removes the nodes
		 * from the DocumentFragment and then we will lose the reference.
		 */
	 ...
	 *   Default is `[]`
	 * @param {Element[]} [next=[]] - Array with next elements. Default
	 *   is `[]`
	 * @param {boolean} [short=false] - Whether to use fast clear.
	 *   Default is `false`
	...
	var _div = createPartial(
		'<div bla bla=true bla=false bla bla bla=0></div>',
	)
	render(
		_div([
			{
