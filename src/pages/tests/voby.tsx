...
type Constructor<T, Args extends unknown[] = unknown[]> = new (
  ...args: Args
) => T
...
const TestSnapshots = ({
  ...
  Component: (JSX.Component | Constructor<Component>) & {
    test: { static?: boolean; wrap?: boolean; snapshots: string[] }
  }
...
const TestProgressIndeterminateToggle = (): JSX.Element => {
  const o = $<number | null | undefined>(0.25)
...
const TestClassesObjectRemoval = (): JSX.Element => {
  const o = $<FunctionUnwrap<JSX.Class> | null>({
    red: true,
    blue: false,
  })
  const toggle = () =>
...
const TestStylesRemoval = (): JSX.Element => {
  const o = $<FunctionUnwrap<JSX.Style> | null>({
    color: 'orange',
    fontWeight: 'normal',
  })
  const toggle = () =>
...
const TestRefUnmounting = (): JSX.Element => {
  ...
  useEffect(
    () => {
      ...
      if (element) {
        message(
          `Got ref - Has parent: ${!!element.parentElement} - Is connected: ${element.isConnected}`,
        )
      } else {
        message(`No ref`)
      }
...
const TestRefContext = (): JSX.Element => {
  ...
  const Reffed = (): JSX.Element => {
    const ref = element =>
      message(
        `Got ref - Has parent: ${!!element.parentElement} - Is connected: ${element.isConnected} - Context: ${useContext(Context)}`,
      )
...
const TestPromiseResolved = (): JSX.Element => {
  const resolved = usePromise<string>(
    new Promise(resolve =>
  ...
  return (
    <>
      ...
      {() => {
        ...
        if (resolved().error)
          return <p>{resolved().error!.message}</p>
...
const TestPromiseRejected = (): JSX.Element => {
  const rejected = usePromise<number>(
  ...
  return (
    <>
      ...
      {() => {
        ...
        if (rejected().error)
          return <p>{rejected().error!.message}</p>
...
const TestTemplateExternal = (): JSX.Element => {
  const Templated = template<{ class: string; color: string }>(
    props => {
      return (
...
const TestRenderToStringSuspenseNested =
...
const TestLazy = (): JSX.Element => {
  ...
  const lazyFetcher = () =>
    new Promise<{ default: JSX.Component }>(resolve =>
      setTimeout(
