...
import $, {
  ...
  batch as batchOby,
  root as rootOby,
} from 'https://cdn.jsdelivr.net/npm/oby/dist/index.js'
...
function testMutable(lib, _test, mutable, memo, batch, signal, root) {
  ...
  if (lib === 'pota: ') {
    ...
    ;[identity, mutable].map(mutable => {
      test(lib + 'reconcile replace - a simple object', expect => {
        const state = mutable({
          data: 2,
          missing: 'soon',
        })
      ...
      test(
        lib + 'reconcile replace - a super simple object',
        expect => {
          const state = mutable({
            missing: 'soon',
          })
      ...
      test(lib + 'reconcile replace - array with nulls', expect => {
      ...
      test(
        lib + 'reconcile replace - a simple object on a nested path',
        expect => {
          const state = mutable({
            data: {
              user: {
      ...
      test(
        lib +
          'reconcile replace - a simple object on a nested path with no prev state',
        expect => {
          const state = mutable({})
      ...
      test(
        lib + 'reconcile replace - reorder a keyed array',
        expect => {
      ...
      test(
        lib + 'reconcile replace - overwrite in non-keyed',
        expect => {
      ...
      test(
        lib + 'reconcile replace - top level key mismatch',
        expect => {
          const JOHN = { id: 1, firstName: 'John', lastName: 'Snow' }
          const NED = { id: 2, firstName: 'Ned', lastName: 'Stark' }
      ...
      test(
        lib + 'reconcile replace - top level key missing',
        expect => {
          const store = mutable({
            id: 0,
            value: 'value',
      ...
      test(
        lib + 'reconcile replace - overwrite an array with an object',
        expect => {
          const store = mutable({
            value: [1, 2, 3],
          })
      ...
      test(
        lib + 'reconcile merge - adding and modifying property',
        expect => {
      ...
      test(
        lib + 'reconcile merge - change array to object',
        expect => {
          const target = mutable({ a: true, q: [1, 2, 3, 4] })
      ...
      test(
        lib + 'reconcile merge - change object to array',
        expect => {
      ...
      test(
        lib + 'reconcile merge - add new item using keys',
        expect => {
      ...
      test(
        lib + 'reconcile merge - add new item using keys nested',
        expect => {
      ...
      test(
        lib +
          'reconcile merge - add new item using keys nested and modify',
        expect => {
          const target = mutable({
            q: {
      ...
      test(
        lib + 'reconcile merge - add 2 new items using keys',
        expect => {
      ...
      test(
        lib + 'reconcile merge - merge and add new item using keys',
        expect => {
          const target = mutable({
            a: true,
            q: [1, 2],
      ...
      test(lib + 'reconcile merge - deep test ', expect => {
        const target = mutable({
          a: true,
          q: [1, 2],
      ...
      test(
        lib + 'reconcile replace - add new item using keys',
        expect => {
          const target = mutable({ c: [{ id: 1 }] })
      ...
      test(
        lib + 'reconcile replace - add new item using keys nested',
        expect => {
          const target = mutable({
            q: { u: { a: { c: { k: [{ d: [{ id: 1 }] }] } } } },
          })
      ...
      test(
        lib +
          'reconcile replace - add new item using keys nested and modify',
        expect => {
          const target = mutable({
            q: {
              u: { a: { c: { k: [{ d: [{ id: 1, name: 'a' }] }] } } },
      ...
      test(
        lib + 'reconcile replace - add 2 new items using keys',
        expect => {
          const target = mutable({ c: [{ id: 1 }], d: [{ idx: 2 }] })
      ...
      test(
        lib + 'reconcile replace - delete items using keys',
        expect => {
          const target = mutable({ c: [{ id: 1 }], d: [{ idx: 2 }] })
      ...
      test(
        lib +
          'reconcile replace - replace and add new item using keys',
        expect => {
          const target = mutable({
            a: true,
            q: [1, 2],
      ...
      test(
        lib +
          'reconcile replace - replace, add and delete, using keys',
        expect => {
          const target = mutable({
            a: true,
            q: [1, 2],
