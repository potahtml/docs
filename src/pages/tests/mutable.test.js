...
function testMutable(lib, effect, mutable, memo, batch, signal, root) {
	lib = lib + ": ";
	console.log(lib);
	...
	test(lib + "mutation: object frozen within frozen nested [oby]", (expect) => {
		const source = mutable(
			Object.freeze({
				data: Object.freeze({
	...
	test(
		lib + "mutation: returned object by function call is mutable",
		(expect) => {
			const result = mutable({
				fn: () => ({
					cat: "quack",
	...
	test(lib + "getters: returning frozen object nested [solid]", (expect) => {
		let value = "quack";
		const result = mutable({
			get greeting() {
	...
	test(
		lib + "getter/setters: class, should fail when trying to set in a getter",
		(expect) => {
			class Cat {
				#name = "quack";
				get name() {
	...
	test(lib + "deleting: should throw when non-configurable", (expect) => {
		const result = mutable({});
		...
		try {
			delete result.cat;
			expect("to throw when deleting a non-configurable property").toBe(true);
		} catch (e) {}
	});
	...
	test(
		lib + "delete non existent key doesnt trigger reactivity - value [solid]",
		(expect) => {
			const result = mutable({ a: "somevalue" });
			expect("a" in result).toBe(true);
			expect("b" in result).toBe(false);
	...
	test(
		lib + "delete key with defined value does trigger reactivity - reading ",
		(expect) => {
			const result = mutable({ a: "somevalue", b: undefined });
			expect("a" in result).toBe(true);
			expect("b" in result).toBe(true);
	...
	test(lib + "in: getters to not be called 3 [solid, oby]", (expect) => {
		let access = 0;
		const result = mutable({
			a: 1,
			get b() {
	...
	test(lib + "in: getters to not be called 4 [solid, oby]", (expect) => {
		let access = 0;
		const result = mutable({
			a: 1,
			get b() {
	...
	test(lib + "track: undefined value", (expect) => {
		...
		/**
		 * Tricky because signal gets deleted(see previous lines), then we add it
		 * again with the following, but the signal is not the same one as before,
		 * so effect doesnt re-trigger
		 */
		result.name = "mishu";
	...
	test(lib + "read and set inside extended class [solid, oby]", (expect) => {
		class Tests2 {
			a = 1;
			get b() {
				return this.a * 4;
	...
	test(lib + "read and set inside extended x2 class [solid, oby]", (expect) => {
		class Test4 {
			get b() {
				return this.a * 4;
			}
	...
	test(lib + "reacts to hasOwnProperty", (expect) => {
		let m = mutable({ a: { deep: "test" } });
	...
	test(
		lib + "is both a getter and a setter, for shallow primitive properties",
		(expect) => {
	...
	test(
		lib + "is both a getter and a setter, for shallow non-primitive properties",
		(expect) => {
			const obj1 = { foo: 123 };
			const obj2 = {};
	...
	test(
		lib + "is both a getter and a setter, for deep primitive properties",
		(expect) => {
	...
	test(
		lib + "is both a getter and a setter, for deep non-primitive properties",
		(expect) => {
			const obj1 = { foo: 123 };
			const obj2 = {};
	...
	test(
		lib + "creates a dependency in a memo when getting a shallow property",
		(expect) => {
	...
	test(
		lib + "creates a dependency in a memo/effect when getting a deep property",
		(expect) => {
	...
	test(
		lib + "supports wrapping a deep plain object inside a plain object",
		(expect) => {
			const o = mutable({ value: {} });
	...
	test(lib + "supports reacting to deleting a shallow property", (expect) => {
	...
	test(
		lib + "supports reacting when deleting a shallow property that was null",
		(expect) => {
	...
	test(lib + "supports reacting to deleting a deep property", (expect) => {
	...
	test(
		lib + "supports not reacting when setting a primitive property to itself",
		(expect) => {
			const o = mutable({ value: 1 });
	...
	test(lib + "supports reacting to properties read by a getter", (expect) => {
		const o = mutable({
			foo: 1,
			bar: 2,
			get fn() {
	...
	test(
		lib + "supports reacting to properties read by a regular function",
		(expect) => {
			const o = mutable({
				foo: 1,
				bar: 2,
	...
	test(lib + "supports batching setters automatically [oby]", (expect) => {
		const o = mutable({
			foo: 1,
			bar: 2,
			set fn(increment) {
	...
	test(lib + "supports reacting to changes on custom classes", (expect) => {
		class Foo {
			constructor() {
				this.foo = 0;
				return mutable(this);
	...
	test(lib + "survives reading a value inside a discarded root", (expect) => {
		const o = mutable({ value: 123 });
	...
	test(
		lib + "observed value should proxy mutations to original (Object)",
		(expect) => {
			const original = { foo: 1 };
	...
	test(
		lib + "original value change should reflect in observed value (Object)",
		(expect) => {
	...
	test(
		lib + "observing the same value multiple times should return same Proxy",
		(expect) => {
			const original = { foo: 1 };
			const observed = mutable(original);
	...
	// #1246
	test(
		lib + "mutation on objects using reactive as prototype should trigger",
		(expect) => {
	...
	test(lib + "should not observe non-extensible objects [solid]", (expect) => {
		let mutableObj;
		let testObj;
	...
	test(
		lib + "should not be triggered when set with the same proxy [oby]",
		(expect) => {
	...
	test(lib + "should avoid infinite loops with other effects", (expect) => {
	...
	// #1246
	test(
		lib + "mutation on objects using mutable as prototype should trigger",
		(expect) => {
	...
	test(
		lib + "array: supports wrapping a deep array inside a plain object",
		(expect) => {
			const o = mutable({ value: [] });
	...
	test(
		lib + "array: supports wrapping a deep plain object inside an array",
		(expect) => {
			const o = mutable([{}]);
	...
	test(lib + "array: slice test", (expect) => {
		[
			...
			expect(array.slice(2)).toHaveShape(["camel", "duck", "elephant"]);
	...
	test(lib + "array: sliced test [solid]", (expect) => {
		const original = [{ foo: 1 }];
		const result = mutable(original);
	...
	test(
		lib + "array: calls correct identity-sensitive method on Array subclass",
		(expect) => {
			const subArray = new SubArray(4, 5, 6);
			const observed = mutable(subArray);
			let index;
			...
			expect(mutable(observed).slice()).not.toBe(mutable(subArray).slice());
		},
	);
	...
	test(lib + "array: reading length and pusing doesnt loop", (expect) => {
		const result = mutable([]);
	...
	test(
		lib + "array: pushing in two separated effects doesnt loop [solid]",
		(expect) => {
	...
	test(lib + "array: track: array functions read vs write", (expect) => {
	...
	test(lib + "array: supports reacting when array length changes", (expect) => {
		const o = mutable({ value: [0] });
	...
	test(
		lib + "array: supports reacting when array length is set explicity",
		(expect) => {
			const o = mutable({ value: [0] });
	...
	test(
		lib + "array: supports not reacting when array reading function is called ",
		(expect) => {
	...
	test(
		lib + "array: supports not reacting when array writing function is called ",
		(expect) => {
			const o = mutable({ value: [0, 1] });
	...
	test(lib + "array: supports reacting to changes in deep arrays", (expect) => {
	...
	test(
		lib + "array: supports not reacting to changes in top-level arrays",
		(expect) => {
			const o = mutable([1, 2]);
	...
	test(
		lib + "array: supports batching array methods automatically [oby]",
		(expect) => {
	...
	test(
		lib + "array: observed value should proxy mutations to original [solid]",
		(expect) => {
	...
	test(lib + "array: identity methods should work [solid]", (expect) => {
		let og = {};
		let arr;
	...
	test(
		lib +
			"array: internal array functions should search for the mutable versions of it [solid]",
		(expect) => {
			const item1 = { id: 1 };
			const item2 = { id: 2 };
	...
	test(
		lib + "array: delete on Array should not trigger length dependency",
		(expect) => {
	...
	// #2427
	test(lib + "array: track length on for ... in iteration", (expect) => {
	...
	// #9742
	test(lib + "array: mutation on user proxy of reactive Array", (expect) => {
		const array = mutable([]);
	...
	test(lib + "array: should observe sparse array mutations", (expect) => {
		let dummy;
		const list = mutable([]);
		list[1] = "World!";
		effect(() => (dummy = list.join(" ")));
	...
	test(
		lib + "array: should not observe well-known symbol keyed properties",
		(expect) => {
	...
	test(
		lib + "array: should trigger all effects when array length is set to 0",
		(expect) => {
	...
	test(
		lib +
			"array: should avoid infinite recursive loops when use Array.prototype.push/unshift/pop/shift [solid]",
		(expect) => {
	...
	test(lib + "array: vue array instrumentation: find and co.", (expect) => {
	...
	test(
		...
		(expect) => {
			...
			expect(reactive.reduce((acc, x) => acc + "" + x.val, undefined)).toBe(
				"undefined12",
			);

			let left = memo(() => reactive.reduce((acc, x) => acc + "" + x.val));
			let right = memo(() =>
	...
	// Node 20+
	test(lib + "array: vue array instrumentation: toReversed", (expect) => {
		// mutable
	...
	test(lib + "array: vue array instrumentation: toSpliced", (expect) => {
		const array = mutable([1, 2, 3]);
