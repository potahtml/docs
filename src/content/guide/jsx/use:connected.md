...
Element attribute that runs its callback once the element is
connected to the document. Available only as an attribute on elements
(not as a standalone lifecycle — that's [ready](/ready)). Accepts a
single callback or an array of callbacks; each receives the element.
...
function App() {
	return (
		<main
			use:connected={node => console.log(node, 'connected')}
		>
			Content
		</main>
	)
