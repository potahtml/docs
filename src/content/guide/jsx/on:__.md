...
subpath: pota
topic: Events
desc: Attach a native or custom event listener, with optional listener options or multiple handlers.
---
...
standalone code, tied to the reactive scope) see
...
function App() {
	...
	return (
		<main>
			<label>
				Filter list:{' '}
				<input on:input={filter} placeholder="Filter" />
			</label>
			<ul>
