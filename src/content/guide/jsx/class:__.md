...
subpath: pota
topic: CSS
desc: Set CSS classes as a string, array, object, or namespaced per-class toggle.
---
...
When you swap the object on a `class={...}` binding, classes that
were on the previous object but not on the new one are **not**
removed — object forms add and update, they don't sweep. To clear a
class, set its value to `false` explicitly.
...
function App() {
	return (
		<main>
			...
			<div class="orange" class:orange={undefined}>
				namespace undefined keeps orange
			</div>
			<div class="orange" class:red={true}>
...
function App() {
	...
	return (
		<main>
			...
			<button on:click={() => active.update(v => !v)}>
				toggle
			</button>
		</main>
	)
