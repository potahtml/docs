...
The callback is registered as a [cleanup](/cleanup) on the
surrounding reactive scope, so it fires when that scope disposes —
when the component unmounts or its owning [render](/render) call is
disposed.
...
runs the registered cleanup — so the `use:disconnected` callback fires.
