...
subpath: pota
topic: Props
desc: Imperatively assign a DOM property on an element, unwrapping reactive accessors.
---

# setProperty

Imperatively assign a DOM property on an element, unwrapping
reactive accessors. Assigning `null` or `undefined` sets the
property to `null`. The declarative form is `prop:__`; for HTML
attributes specifically see [setAttribute](/setAttribute).

Use `setProperty` (or the `prop:` namespace in JSX) whenever the DOM
property and the HTML attribute diverge — `value` on inputs after
user edits, `checked` state, `srcObject` on a `video`, or custom
...
attribute), so it is driven from a signal — pressing _min_/_max_
snaps the slider instantly.
...
function App() {
	...
	return (
		<div>
			<input use:ref={slider} type="range" min="0" max="100" />
