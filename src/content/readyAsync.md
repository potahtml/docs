...
subpath: pota
topic: Renderer
desc: Schedule a callback to run once every tracked async task has settled.
---

# readyAsync

`readyAsync(fn)` waits for any tracked async work (promises inside
[`derived`](/derived) / `withValue` / etc.) to complete before
firing. Useful when you need a snapshot of the world _after_
everything has loaded — common in tests or in screenshot / printing
flows. For the synchronous form see [`ready`](/ready).
...
in-flight tasks all settle before the callback runs. If new async
work is still outstanding when the queue drains, the callbacks stay
queued until everything settles.
