...
subpath: pota
topic: Reactive core
desc: Signal for arrays of objects with an id key whose write patches by id, reusing deep-equal entries by reference.
---
...
reaching for a full [store](/store/store) — downstream code that compares by
reference (`===`), such as a keyed [`<For/>`](/components/For), sees a
minimal diff.
...
done) so it becomes a new reference, `id 2` stays deep-equal so the old
reference is reused, and `id 3` is new. The keyed `<For/>` only touches
