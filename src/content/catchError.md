...
subpath: pota
topic: Reactive core
desc: Runs a function inside an error boundary, routing any thrown error — sync or reactive — to a handler instead of the console.
---
...
chain — `handler(err)` is called and the error does not bubble or reach
the console. Returns the value of `fn`, or `undefined` if it threw.
...
which writes the message into a signal instead of letting it surface on
the console.
