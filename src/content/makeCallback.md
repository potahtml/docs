...
subpath: pota
topic: Renderer
desc: Wraps children into a callable that re-evaluates them with the arguments you pass, untracking marked components.
---
...
arguments you pass. It's the helper flow components like
[`<Show/>`](/components/Show), [`<Switch/>`](/components/Switch),
and [`<For/>`](/components/For) use to support children that are
either static JSX or render functions like `{(item, index) => …}`.
...
function App() {
	return (
		<HoverPanel>{state => <p>state is {state}</p>}</HoverPanel>
	)
}
