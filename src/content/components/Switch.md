...
Renders the first [`<Match/>`](/components/Match) child whose `when` is
truthy. If none match, renders `fallback` — or, if no explicit
...
signal carrying the current `when` value — the same callback pattern as
...
`<Switch/>` renders the first branch whose `when` is truthy. Here every
`when` is falsy (`0`, `undefined`, `false`), so the `fallback` wins.
	...
	return (
		<div>
			...
			<Switch>
				...
				<Match
					when={() =>
						result.read().kind === 'err' && result.read()
					}
				>
