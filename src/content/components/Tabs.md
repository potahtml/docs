...
topic: Layout
desc: Accessible, nestable tabs built from Labels, Label, Panels, and Panel sub-components.
---
...
via `aria-labelledby` and paired by position. The first tab is active by default; pass `selected={index}`
on `<Tabs>` to start elsewhere. Unknown props are forwarded to the
underlying element.
	...
	return (
		<Tabs>
			...
			<Tabs.Labels>
				...
				<Tabs.Label name="two" selected>
					two
				</Tabs.Label>
				<Tabs.Label name="three" hidden>
					three
				</Tabs.Label>
			</Tabs.Labels>
			<Tabs.Panels>
