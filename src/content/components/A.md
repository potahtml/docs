...
topic: Routing
desc: Anchor that intercepts clicks for client-side Route navigation, with param substitution.
---

# `<A/>`

Anchor that intercepts clicks for client-side [`<Route>`](/components/Route)
navigation. Renders a regular `<a>` but resolves its `href` relative to
the current route — when `href` is not absolute (does not start with `/`,
`#`, or `http`) it is made relative to the route the link sits in.
...
All other props (`class`, `target`, `on:click`, …) are forwarded to the
underlying `<a>`.
...
substitutes `:id` in the `href`, and `replace` swaps history instead of
pushing a new entry.
...
characters survive in the resolved `href`. Placeholders with no matching
...
function App() {
	return (
		<ul>
			...
			<li>
				<A href="#:cat/:page" params={{ cat: 'variété', page: 'touché' }}>
					hash link
				</A>
			</li>
			<li>
				<A href=":nope" params={{ page: 'touché' }}>
