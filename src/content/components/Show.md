...
topic: Flow
desc: Renders its children when the when condition is truthy, else a fallback.
---
...
function App() {
	...
	return (
		<div>
			<button on:click={() => showing.update(v => !v)}>
				toggle
			</button>
...
Wrapping a component in [resolve](/resolve) memoizes the result, so the
component is created once and reused across toggles instead of being
re-rendered each time `Show` swaps back in.
...
function App() {
	...
	return (
		<div>
			...
			<Show when={showing.read} fallback={<p>nothing here</p>}>
				Hey
			</Show>
		</div>
	)
