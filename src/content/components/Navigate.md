...
[`<Show>`](/components/Show) to bounce the user when a condition is met.
...
route matches. `replace` swaps the history entry so the old URL doesn't
		...
		<div>
			...
			<Route path="/protected">
				<Show
					when={loggedIn.read}
					fallback={
						<Navigate path="/login" delay={1000}>
