...
topic: Text
desc: Inline formatter for chat-style text — markers, media URLs, emoji shortcodes, and a highlight term.
---
...
A leading `>` wraps the whole block in `<q>`; a leading `/ ` italicises
the whole block. Markers may nest; whitespace inside a marker is
preserved.
...
function App() {
	...
	return (
		<div>
			...
			/>
			<p>
				{() => <Linkify text={text.read()} mark={term.read()} />}
			</p>
		</div>
	)
}
...
With `emoji`, `:shortcode:` sequences are replaced with unicode
emoji.
...
function App() {
	return (
		<p>
			<Linkify text="shipping it :rocket: with a :smile:" emoji />
		</p>
	)
}
