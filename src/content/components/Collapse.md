...
topic: Flow
desc: Like Show, but hides its children with display:none instead of unmounting them.
---
...
(`style:display` flips to `none`) rather than removed from the document.
State inside the children survives across hide/show, which makes it the
right tool for iframes, canvases, video, audio, or any expensive widget
you don't want to re-mount.

While `when` is falsy, an optional `fallback` is rendered in place of the
hidden children.
