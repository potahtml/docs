...
relative to that route, so `uno/` from within `/` navigates to `/uno/`.
`<Route.Default/>` renders when no sibling route matched.
...
re-render — useful for iframes, canvas, video, or audio that would lose
state on remount. The frame keeps playing as you navigate away and
back.
...
function App() {
	return (
		<main>
			...
			<Route path="/#">
				<Route path="kilo/">
					<Route path="frame" collapse={true}>
