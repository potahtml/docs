...
topic: Document
desc: Mounts children into document.head, deduplicating title, meta, and rel=canonical.
---

# `<Head/>`

Mounts its children into `document.head`. When a new child is
inserted there the runtime detects tags that should be unique and
replaces the existing one in place, so multiple pages/components can
all contribute to the head without duplicates.
