...
subpath: pota
topic: Events
desc: Imperative addEventListener tied to the reactive scope — returns an off() and auto-detaches on disposal.
---
...
so the listener is torn down automatically when the scope is
disposed — you don't have to call `off()` manually in most cases. Its
