...
subpath: pota/store
topic: Store
desc: Turn an object's own properties into tracked signal getters/setters in place, without recursing.
---

# signalify

Like [mutable](/store/mutable), but shallow: only the target's own
properties become tracked getters/setters. Nested objects stay plain
objects. Useful for opt-in reactivity on a handful of keys without
walking the whole tree — for example signalifying class instance
fields. Each key becomes a [signal](/signal) behind the scenes.
...
signalified — and those keys don't have to exist yet.

## Arguments

| name    | type           | description                                                      |
| ------- | -------------- | ---------------------------------------------------------------- |
| `target`| `T`            | object whose own properties are turned into signals, in place    |
...
Without a `keys` argument, every own property of the target is
signalified — reading `state.count` in a reactive context re-runs
when it changes.
...
const state = signalify(
	{ firstName: 'Quack', lastName: 'Murci' },
	['lastName'],
)
