...
subpath: pota/store
topic: Store
desc: Reconcile source into target in place, keeping target keys that are absent from source.
---
...
non-removing merge of nested arrays by key see
[merge](/store/merge).
...
part of it back to those defaults. Note the `mutable(defaultState,
true)` copy — otherwise the mutable proxy would share references with
`defaultState`.
