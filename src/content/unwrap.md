...
subpath: pota
topic: Reactive core
desc: Recursively calls any function it encounters and flattens arrays one level, returning a plain snapshot value.
---
...
Reach for it only when you genuinely need a one-shot snapshot outside a
tracking scope. For any reactive use prefer [resolve](/resolve), which
memoises the walk so subsequent reads are cheap and consumers re-run on
change.
