...
subpath: pota
topic: Renderer
desc: Turns any value into a callable component factory, optionally with preset props.
---
...
factory time; calling the factory with more props shallow-merges
them over the presets (_later wins, key-by-key_). For the JSX
equivalent see [`<Dynamic/>`](/components/Dynamic); for
class-style components, see [`Pota`](/Pota).

When the second argument is omitted, `Component` just wraps `value`
so it can be invoked imperatively and keep receiving fresh props on
each call.
