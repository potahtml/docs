...
subpath: pota
topic: Reactive core
desc: Like effect, but the body runs synchronously when a dependency changes instead of on the next scheduler tick.
---
...
tick. Pick it only when the side-effect must land before the
current call returns — for everything else, `effect` is the right
primitive.
