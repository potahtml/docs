...
subpath: pota
topic: Reactive core
desc: Creates a signal-function for capturing a DOM element via the use:ref attribute.
---
...
arguments and writes when invoked with one — `r()` returns the
current value, `r(node)` stores it. `ref()` is the signal-function
shape specialised for refs, with no initial value.
...
**Returns:** a signal-function — call it with no args to read
(`r()`), call it with a value to write (`r(value)`). The renderer
writes the element into it when used as `use:ref`.
...
function App() {
	...
	return <input placeholder="auto-focused" use:ref={input} />
}
