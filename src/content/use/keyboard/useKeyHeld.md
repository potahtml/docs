...
non-reactive live set use [`keysHeld`](/use/keyboard/keysHeld). Part of
...
**Returns:** `() => boolean` — a reader that is `true` while the key is
held.
...
function App() {
	...
	return (
		<div>
			<p>direction: {() => (y() === 0 ? 'idle' : y() < 0 ? 'up' : 'down')}</p>
