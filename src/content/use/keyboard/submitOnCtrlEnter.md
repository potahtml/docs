...
`submitOnCtrlEnter(fn)` is a `use:ref` convenience that calls `fn` when
the user presses Ctrl+Enter (Cmd+Enter on Mac) on the element — the
common "send on Ctrl+Enter" affordance for textareas. It is a thin
