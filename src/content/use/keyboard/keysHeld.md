...
[`useAnimationFrame`](/use/animate) loops where reactive tracking would
be wasted overhead: WASD movement, hold-to-charge meters, modifier
inspection during a drag. The set is mutated in place by the shared
tracker; treat it as read-only. For a reactive per-key boolean use
[`useKeyHeld`](/use/keyboard/useKeyHeld). Part of
...
function App() {
	...
	return (
		<p>
			hold <kbd>W</kbd>/<kbd>A</kbd>/<kbd>S</kbd>/<kbd>D</kbd> — position{' '}
			{() => `(${x.read()}, ${y.read()})`}
		</p>
	)
}
