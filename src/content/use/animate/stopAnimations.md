...
topic: Animation
desc: Cancel every animation running on an element; returns the canceled list.
---
...
from `element.getAnimations()`). Each has a `.finished` promise — `await
Promise.all(list.map(a => a.finished.catch(() => 0)))` to wait for the
cancellations to settle.
...
function App() {
	...
	return (
		<>
			...
			<div use:ref={box} class="box pulse">
				box
			</div>
		</>
	)
