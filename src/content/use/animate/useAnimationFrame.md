...
topic: Animation
desc: Owned requestAnimationFrame loop with start/stop, auto-stopped on dispose.
---
...
calls can be chained. `start()` restarts the loop; `stop()` cancels the
pending frame.
...
hundred. Calling `stop()` from inside `fn` cancels the already-scheduled
next frame, so the loop halts on the same tick.
