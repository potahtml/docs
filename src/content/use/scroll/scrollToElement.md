...
`scrollToElement(node)` resets the node's `scrollTop` to `0` and then calls
`node.scrollIntoView(true)` — an imperative scroll to a node you already hold.
Part of [`pota/use/scroll`](/use/scroll); for a declarative on-mount form use
...
Holds a ref to a far-down section and scrolls to it imperatively when the
button is clicked.
