...
`window.location.hash`, if one exists — useful after a client-side navigation
that changes the hash. It is a thin wrapper over
[`scrollToSelector`](/use/scroll/scrollToSelector) passing `location.hash`.
...
**Returns:** `true` when the hash matched an element, `false` otherwise.

## Examples

### Honor the URL hash on a button press

Renders an anchored section and a button that re-scrolls to whatever the
current `location.hash` points at.
