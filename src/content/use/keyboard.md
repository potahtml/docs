...
followed by a single key. Matching is strict: `ctrl+s` does _not_
fire on `ctrl+shift+s`. The handler is called with the
`KeyboardEvent`; `preventDefault` is applied automatically when the
chord matches.
...
- [`useKeyHeld(key)`](/use/keyboard/useKeyHeld) — reactive boolean:
  is a key held?
