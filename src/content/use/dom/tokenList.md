...
topic: Internals
desc: Trim and split a string by whitespace; falsy → shared empty array.
---

# tokenList

`tokenList(s)` trims and splits a string by whitespace; for falsy or
whitespace-only input it returns the module-shared empty array. Used by
[`addClass`](/use/dom/addClass) /
[`removeClass`](/use/dom/removeClass). Part of [`pota/use/dom`](/use/dom).
