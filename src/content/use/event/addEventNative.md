...
topic: Events
desc: addEventListener wrapper that accepts a handler object as its options.
---
...
Registers a passive `wheel` listener on `window`, then removes it with the
same handler reference. Passing a [passiveEvent](/use/event/passiveEvent)
object hands the options bag and the handler to `addEventListener` in one go.
