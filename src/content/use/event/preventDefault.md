...
`preventDefault(e)` calls `e.preventDefault()` — drop it straight
into an `on:*` handler. For all three stop methods at once use
...
Drops `preventDefault` straight into an `on:click` handler so the anchor never
navigates.
