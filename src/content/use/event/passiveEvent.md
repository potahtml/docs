...
[`addEventNative`](/use/event/addEventNative). Passive listeners
can't call `preventDefault`, which lets the browser scroll without
waiting on your handler. Part of [`pota/use/event`](/use/event).

## Examples

### A passive wheel listener

Wraps a `wheel` handler so the browser can scroll without waiting on it, then
attaches it with [addEventNative](/use/event/addEventNative).
