...
Call it **within a reactive scope** (a component body, [effect](/effect),
or [root](/root)). The subscription to `visibilitychange` is created on
first use through the surrounding scope and torn down automatically
when that scope is cleaned up; multiple consumers share the one
underlying listener. Calling it outside any scope leaks the listener.
...
tab is shown or hidden. Because `useDocumentVisible()` is called in the
component body, its listener is registered and cleaned up with the
component.
