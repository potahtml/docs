...
topic: Input
desc: Reactive gamepad connect, button, trigger, and axis state, plus a snapshot.
---
...
`requestAnimationFrame` poll that starts on the first subscription
and stops once the last consumer disposes. Even connect / disconnect
is derived from the poll — one source of truth. Each reactive
accessor lazily allocates its backing signal the first time it's
called for a given (gamepad, button/axis) pair, so the poll only
updates signals consumers actually subscribed to.
