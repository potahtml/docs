...
dropped on it run through the same pipeline as
[`upload`](/use/upload) (filters → upload → callbacks). While files
are dragged over, the element gets a `data-dragover` attribute —
style it with `[data-dragover]`. The standard `dragenter` /
`dragleave` counter pattern is used so crossing child elements
doesn't strip the attribute mid-hover. Part of
[`pota/use/upload`](/use/upload).
