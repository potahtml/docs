...
`sleep(ms = 0)` is `setTimeout(_, ms)` wrapped in a promise — `await
sleep(50)` to pause. The fixed longer wait is
[`sleepLong`](/use/test/sleepLong). Part of [`pota/use/test`](/use/test).
