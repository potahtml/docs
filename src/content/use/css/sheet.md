...
`sheet(css)` creates a `CSSStyleSheet` from a CSS string. It is
cached by source string — the same input always returns the same
...
Identical inputs share one `CSSStyleSheet`, so repeated calls are free.
