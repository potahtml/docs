...
topic: Styling
desc: Add one stylesheet from a URL or CSS string (fetch-or-parse, cached).
---
...
is passed through [`sheet()`](/use/css/sheet). The result is cached, so
one request services every custom element that wants the same URL and
all of them reference the same object. The resolved sheet is then
