...
topic: Routing
desc: Substitute :name placeholders in a route pattern with URL-encoded values.
---

# replaceParams

`replaceParams(href, params)` substitutes named `:name` placeholders in
a route pattern with URL-encoded values from `params`. Placeholders are
matched by [`paramsRegExp`](/use/url/paramsRegExp)
(`/\:([a-z0-9_\-]+)/gi`). Keys missing from `params` are left intact, so
partial substitution works. This is the same primitive
...
`encodeURIComponent` form. When `params` is omitted, `href` is returned
unchanged.
