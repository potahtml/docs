...
Reactive number in `[-1, 1]` for a single stick axis. The value is
raw — apply your own deadzone where you need one
...
an inline style. A small deadzone keeps the dot still when the stick is
centered. The reader functions go straight into the reactive `left` /
`top` style wrappers.
