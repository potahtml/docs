...
[`pota/use/clickoutside`](/use/clickoutside) that calls `handler`
when the user presses Escape. It listens for `keydown` on `document`
and fires only when `e.key === 'Escape'`. The element itself is passed
to the handler so one closure can serve several elements. Attach it
with `use:ref`, and compose it with other factories such as
