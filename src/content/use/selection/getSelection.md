...
`window.getSelection()` and returns its first range
(`getRangeAt(0)`). Pair it with
[`restoreSelection`](/use/selection/restoreSelection) when you have to
mutate text the user is in the middle of editing. Part of
[`pota/use/selection`](/use/selection).
