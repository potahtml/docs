...
topic: Interaction
desc: Imperatively step focus backward through focusable elements, with wrap-around.
---
...
wrapping back to the end when it reaches the start — the counterpart to
[`focusNext`](/use/focus/focusNext). Called with no arguments it scans
the whole document; pass an explicit array to scope the cycle to a
subset. Part of [`pota/use/focus`](/use/focus).
...
function App() {
	return (
		<form>
			...
			<button type="button" on:click={() => focusPrevious()}>
				prev
			</button>
			<button type="button" on:click={() => focusNext()}>
				next
			</button>
		</form>
	)
