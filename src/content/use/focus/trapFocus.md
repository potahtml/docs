...
topic: Interaction
desc: use:ref function confining Tab navigation to an element's descendants.
---

# trapFocus

`trapFocus` confines `Tab` / `Shift+Tab` navigation to the focusable
descendants of the element — a standard accessibility need for
modals, popovers, and command palettes. It does nothing when the
element has no focusable descendants. Part of
[`pota/use/focus`](/use/focus).

It installs its own `keydown` handler on the element: on each `Tab`
it queries the element's focusable descendants and, when focus is at
a boundary, prevents the default and wraps — `Tab` on the last
element focuses the first, `Shift+Tab` on the first focuses the last.
This is self-contained; it does not use the document-wide
