...
Picks a different layout per orientation with [`Show`](/components/Show),
driving its `when` from the accessor.
...
function App() {
	...
	return (
		<Show when={isWide} fallback={<aside>stacked layout</aside>}>
