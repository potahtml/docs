...
`useStopwatch(opts?)` returns `{ elapsed, running, start, stop, reset }`.
`elapsed()` is a reactive reader in milliseconds; the underlying tick
is `opts.interval` (default `1000`). For finer resolution lower the
