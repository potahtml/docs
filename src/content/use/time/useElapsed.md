...
topic: Utilities
desc: Reactive seconds-elapsed reader that re-renders only on unit boundaries.
---
...
an hour, once per hour under a day, and so on — so a row that says
_"5 days ago"_ doesn't re-render every second. The argument may be a
value or an accessor; when it's an accessor, reads reflect a changed
