...
topic: Input
desc: Document-wide mouse button and pointer tracking via Pointer Events.
---
...
removed when the last one goes away. A `pointerup` always releases
its button (so a button can't get stuck), and a window `blur` —
...
left button drives a [Show](/components/Show); right-clicking would use button
`2` instead.
...
function App() {
	...
	return (
		<Show when={leftDown} fallback={<p>release</p>}>
