...
- [`useBeforeLeave(fn)`](/use/location/useBeforeLeave) — guard that can
  cancel navigation
...
`location` is a frozen object. Most of its members are
[`memo`](/memo) accessors — call them as zero-arg functions
(`location.pathname()`) to read the current value reactively. They are
**not** signals: there is no `.read()` / `.write()` and you cannot
assign to them; use [`navigate`](/use/location/navigate) to change the
URL.
...
`params` is a getter: each access builds a fresh effect + reactive
map owned by the current scope. Capture it once at component setup
...
[`addListeners()`](/use/location/addListeners) ensures `popstate` /
	...
	return (
		<main>
			<Route path="/:page">
				...
				<button
					on:click={() =>
						navigate('/page?q=variété&page=2#section')
					}
				>
					navigate
				</button>
...
function Catalog() {
	...
	return (
		<div>
			...
			<button
				on:click={() => navigate('/catalog/variété/touché')}
			>
				go
			</button>
		</div>
