...
topic: Browser
desc: Current screen orientation ('horizontal' | 'vertical'), derived from documentSize.
---
...
[`documentSize`](/use/resize) — so it tracks window resizes
(including device rotation) without a separate listener. It has the
same [`Emitter`](/use/emitter)-pair shape as
[`fullscreen`](/use/fullscreen) and
