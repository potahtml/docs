...
that value when it looks like an email — at least 6 characters long and
matching `^[^@]+@[^@]+\.[^@]+$`. Anything else returns `false`. The
returned string is the cleaned value, so you can store it directly.
