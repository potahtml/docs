...
entry, so it takes any CSS color string the parser accepts and returns
a `#RRGGBBAA` hex string.
...
Renders the same color at a few alpha levels over a checkerboard so the
