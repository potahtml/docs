...
topic: Utilities
desc: Alpha-composite an overlay color over a background and return the mix.
---

# blend

`blend(background, overlay, opacity, gamma)` mixes `overlay` over
`background` at the given `opacity`, returning the fully-opaque result.
Re-exported from the
[color-bits](https://github.com/romgrk/color-bits) `string`
entry, so both inputs are CSS color strings and the result is a
`#RRGGBBAA` hex string.
...
**Returns:** `string` — a `#RRGGBBAA` hex string (alpha is always `ff`).

## Examples

### Preview an overlay tint

Composites a translucent accent over a card background to see the final
opaque color the user would perceive.
