...
subpath: pota
topic: Reactive core
desc: Effect for async work, serialised by default — each run receives the previous run's promise to await.
---

# asyncEffect

Effect for async work, serialised by default. On each run the
callback receives the promise from the previous run (or
`undefined` on the first); awaiting that promise guarantees one
run finishes before the next begins. For synchronous effects see
[effect](/effect) / [syncEffect](/syncEffect).
