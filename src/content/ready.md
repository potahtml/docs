...
subpath: pota
topic: Renderer
desc: Schedule a callback to run once the current render batch is connected to the DOM, before the next paint.
---
...
i.e. after the full queue of `connected` callbacks has been
processed, and before the next paint. By then, the elements produced
by the scope should already be connected to the document. For a
callback that also waits on tracked async work, see
