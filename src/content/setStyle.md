...
subpath: pota
topic: Props
desc: Imperatively set a single style property on an element, unwrapping reactive values.
---

# setStyle

Imperatively set a single style property on an element, unwrapping
reactive values. A value of `null`, `undefined` or `false` removes
the property. The declarative form is `style:__` (or a `style`
object); for entire stylesheets see [css](/use/css).
