...
subpath: pota
topic: Renderer
desc: Returns true for functions tagged via markComponent — branch on renderable vs. plain getter.
---

# isComponent

`isComponent(value)` returns `true` for things tagged via
[markComponent](/markComponent). The renderer uses the same check;
the helper is exposed for component libraries that need to branch
on "this prop is a renderable" vs. "this prop is a getter".
...
**Returns:** `boolean` — `true` when `value` is a function marked as
a component.
